...
def check_actor_readiness(actor_name: str, namespace: str = "asya-e2e") -> dict:
    ...
    # Check pod status
    output = run_kubectl(
        ["get", "pods", "-l", f"asya.sh/actor={actor_name}", "-o", "json"],
        namespace,
    )
    if output:
        import json
    ...
    output = run_kubectl(["logs", "-l", f"asya.sh/actor={actor_name}", "--tail=50", "-c", "sidecar"], namespace)
    if output:
...
def log_actor_diagnostics(actor_name: str, namespace: str = "asya-e2e"):
    ...
    if not info["deployment_ready"] or info["queue_consumers"] == 0:
        log_pod_logs(f"asya.sh/actor={actor_name}", f"{actor_name} (Not Ready)", namespace, tail=30)
