...
│   ├── asya-operator/       # Kubernetes operator source
...
- `src/asya-operator/internal/controller/runtime_symlink/asya_runtime.py` → Operator reads and injects into ConfigMap
- `testing/integration/operator/testdata/runtime_symlink/asya_runtime.py` → Integration tests use same runtime
